...
class SimplePastController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('user.simplePastQuiz.simplePastHome', compact('notifications', 'userProgress'));
    }
    ...
    public function quest1()
    {
        ...
        $namaStory = 'questcontent3q1';
        $namaQuest = 'questions3q1';
        $namaQuestK = 'questions3q1k';
    ...
    {
        $nilaiKarma = $this->getKarma();
        $namaStory = 'questcontent3q2';
        $namaQuest = 'questions3q2';
        $namaQuestK = 'questions3q2k';
        $story = DB::table('konten')->where('nama', $namaStory)->first();
        $ceritaContent = json_decode($story->content, true);
        if ($nilaiKarma > 4) {
            $quest = DB::table('konten')->where('nama', $namaQuestK)->first();
    ...
    {
        $namaStory = 'questcontent3q3';
        $namaQuest = 'questions3q3';
        $story = DB::table('konten')->where('nama', $namaStory)->first();
        $ceritaContent = json_decode($story->content, true);
        $quest = DB::table('konten')->where('nama', $namaQuest)->first();
        $questions = json_decode($quest->content, true);
