...
class SimplePresentController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('user.simplePresentQuiz.simplePresentHome', compact('notifications', 'userProgress'));
    }
    ...
    {
        $namaStory = 'questcontent1q1';
        $namaQuest = 'questions1q1';
        ...
        $timertotal = 360;
        return view('user.simplePresentQuiz.simplepresent1', compact('ceritaContent', 'questions', 'timertotal'));
    }

    public function quest2()
    {
        $namaStory = 'questcontent1q2';
        $namaQuest = 'questions1q2';
        $story = DB::table('konten')->where('nama', $namaStory)->first();
        $ceritaContent = json_decode($story->content, true);
        $quest = DB::table('konten')->where('nama', $namaQuest)->first();
        $questions = json_decode($quest->content, true);
        $timertotal = 360;
        return view('user.simplePresentQuiz.simplepresent2', compact('ceritaContent', 'questions', 'timertotal'));
    }

    public function quest3()
    {
        $namaStory = 'questcontent1q3';
        $namaQuest = 'questions1q3';
        $story = DB::table('konten')->where('nama', $namaStory)->first();
        $ceritaContent = json_decode($story->content, true);
        $quest = DB::table('konten')->where('nama', $namaQuest)->first();
        $questions = json_decode($quest->content, true);
        $timertotal = 360;
        return view('user.simplePresentQuiz.simplepresent3', compact('ceritaContent', 'questions', 'timertotal'));
    }
}
